        ...
        <bottomPopup v-model="applyShow">
            <div class="bottomApply" style="">
                ...
                <div class="apply_item" @click="applySelect(1)">
                    <svg-icon :iconClass="'aliApply'" :iconStyle="{fontSize:'0.3rem',color:'#ffffff'}"></svg-icon>
                    <span>支付宝</span>
                    <i style="font-size:0.22rem;margin-right:0.2rem;color:red">最高立减10元</i>
                    <svg-icon :iconClass="current_Apply_type==1?'inquiryAgree':'inquiryNoAgree'" :iconStyle="{fontSize:'0.3rem',color:'#ffffff'}"></svg-icon>
                </div>
                <div class="apply_item" @click="applySelect(4)" v-if="isWechatBrowser">
                    <svg-icon :iconClass="'wxApply'" :iconStyle="{fontSize:'0.3rem',color:'#ffffff',}"></svg-icon>
                    <span>微信
                        <i v-if="isPc">（仅支持手机端用户）</i>
                    </span>
                    <svg-icon :iconClass="current_Apply_type==4?'inquiryAgree':'inquiryNoAgree'" :iconStyle="{fontSize:'0.3rem',color:'#ffffff',}"></svg-icon>
                </div>
                <div class="apply_item" @click="applySelect(2)" v-else>
                    <svg-icon :iconClass="'wxApply'" :iconStyle="{fontSize:'0.3rem',color:'#ffffff',}"></svg-icon>
                    <span>微信
                        <i v-if="isPc">（仅支持手机端用户）</i>
                    </span>
                    <svg-icon :iconClass="current_Apply_type==2?'inquiryAgree':'inquiryNoAgree'" :iconStyle="{fontSize:'0.3rem',color:'#ffffff',}"></svg-icon>
                </div>
                ...
                if (res.code == 1) {
                    if (this.isWechatBrowser) {//判断是否是微信浏览器，直接调用微信内支付
                        ...
                        if (this.current_Apply_type == 1) {
                            this.$nextTick(() => {//支付宝
                                this.$router.push({ name: 'useAlipayInWx', query: { order_no: this.formData.order_no, alipayForm: res.data.pay_url, token: localStorage.getItem('token') } })
                            })
                        } else if (this.current_Apply_type == 4) {
                            this.$toast.clear();
                            this.wChat_in_Apply(res.data)
                        }
                    } else {
                                ...
                                // document.forms['wepaysubmit'].submit();
                                 ...
                                 if (this.isWechatBrowser) {//判断是否是微信浏览器，直接调用微信内支付
                                    ...
                                    if (this.$store.state.client == 'h5') {
                                        ...
                                        this.$href(res.data.pay_url, { hidden: true });
                                    }
                                }
                            })
                        }
