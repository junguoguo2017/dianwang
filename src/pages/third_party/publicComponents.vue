<template>
    <div>
        ...
        <div class="homeBtn" v-if="!showSlider">
            <div
                class="someBtn"
                v-if="$store.state.client != 'ios'"
                @click="showCustom"
            >
                <svg-icon
                    :iconClass="'customerService'"
                    :iconStyle="{ fontSize: '0.4rem', color: '#444' }"
                ></svg-icon>
            </div>
            ...
            <div
                class="someBtn"
                v-if="
                    $store.state.client == 'h5' &&
                        $route.name != 'thirdPartyHome' &&
                        showBackHome
                "
                @click="$router.push({ name: 'thirdPartyHome' })"
            >
                <svg-icon
                    :iconClass="'home'"
                    :iconStyle="{ fontSize: '0.4rem', color: '#444' }"
            ...
            <div
                class="someBtn"
                @click="backClick"
                v-if="$store.state.client == 'h5' && showBack"
            >
                <svg-icon
                    :iconClass="'back'"
                    :iconStyle="{ fontSize: '0.4rem', color: '#444' }"
        ...
        <!--客服-->
        <flypopup
            v-model="$store.state.customShow"
            :popupStyle="{ borderRadius: '0.2rem' }"
        >
            <div class="customBox">
                <div class="select">
                    <span
                        class="item"
                        :class="customItem == 1 ? 'active' : ''"
                        @click="customItem = 1"
                        >客服微信</span
                    >
                    <span
                        class="item"
                        :class="customItem == 2 ? 'active' : ''"
                        @click="customItem = 2"
                        >公众号</span
                    >
                </div>
                <div class="content" v-show="customItem == 2">
                    <div class="code">
                        <img :src="wechat.h5_open_wechat_qrcode" alt="" />
                    </div>
                    <div class="copyBtn">
                        <span @click="copyName(1)">复制公众号并关注</span>
                    </div>
                </div>
                <div class="content" v-show="customItem == 1">
                    <div class="code">
                        <img :src="wechat.wechat_qrcode" alt="" />
                    </div>
                    <div class="copyBtn">
                        <span @click="copyName(2)">复制微信号并联系</span>
                    </div>
                </div>
            </div>
        </flypopup>
            ...
            <div class="phoneinput_history">
                ...
                <div style="margin-top: 0.4rem;" class="history_inputArea">
                    ...
                    <img
                        ...
                        alt=""
                        style="width: 1.5rem"
                        @click="mathNumber = new Date().getTime()"
                    />
                </div>
                <div class="history_inputArea" style="margin-top: 0.4rem;">
                    ...
                    <input
                        ...
                        placeholder="请输入验证码"
                        style="flex-grow: 1;margin-left: 0.2rem"
                    />
...
export default {
    ...
    created() {
        ...
        concatCustomer().then(res => {
            if (res.code == 1) {
                this.open_wechat.open_wechat_qrcode =
                    res.data.service.open_wechat_qrcode;
                this.open_wechat.open_wechat_name =
                    res.data.service.open_wechat_name;
                this.wechat.wechat_qrcode = res.data.service.wechat_qrcode;
                this.wechat.wechat_name = res.data.service.wechat;
                this.wechat.h5_open_wechat_qrcode =
                    res.data.service.h5_open_wechat_qrcode;
    ...
    methods: {
        ...
        showCustom() {
            ...
            if (!window.qimoChatLoaded) {
                let url =
                    "https://ykf-webchat.7moor.com/javascripts/7moorInit.js?accessId=a1a4f190-96fb-11e9-9ff0-b3ba7198c4cf&autoShow=false&language=ZHCN";
...
.homeBtn {
    ...
    .someBtn {
        width: 0.8rem;
        height: 0.8rem;
        background: rgba(89, 109, 255, 0.2);
