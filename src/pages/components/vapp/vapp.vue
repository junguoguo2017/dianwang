<!--<template>
    <div id="app">
        <div ref="content" class="content">
            <div ref="refreshTips" class="refreshIcon">
                <img  v-if="refreshing" src="./refresh_loading.png" class="lodingAnimat">
                <img  v-else-if="refreshend" src="./refreshend.png">
                <img v-else src="./refreshJT.png" alt="" :style="{transform:isRefresh?'rotate(180deg)':'rotate(360deg)'}">
                <span>{{isRefreshtips}}</span>
            </div>
            <slot></slot>
        </div>
    </div>
    ...
    export default {
        ...
        mounted(){
            ...
            function scrolling(){
                ...
                self.bodyScroll(self.bodyDom,function(val){
                    self.$emit('scrolling',val)
                },function(){
                    self.$emit('loadMore')
                })
            };
            /*上拉加载*/
            window.onscroll=function(){//兼容处理
                scrolling()
            }
        ...
        updated(){
            this.$nextTick(()=>{
                this.maxTouchendY = this.$refs.refreshTips.offsetHeight;
            })
        },
        methods:{
            reset(){
            ...
            touchmove(e){
                ...
                if(scrollTop<=0){
                    ...
                    this.touchEndY = (e.touches[0].clientY-this.touchStartY)*0.4;
                    this.touchEndX = e.touches[0].clientX-this.touchstartX;
                    function getangle(diff_y,diff_x){
                        //返回角度,不是弧度
                        return 360*Math.atan(diff_y/diff_x)/(2*Math.PI);
                    };
                    var angle = getangle(Math.abs(this.touchEndY),Math.abs(this.touchEndX))

                    if(0<angle&&angle<20){//设定手指滑动方向,防误触
                        e.preventDefault();
                        e.stopPropagation();
                        this.dropdown = false
                        return
            ...
            refreshDone(){
                ...
                setTimeout(()=>{
                    self.touchEndY=0;
                    self.$refs.content.style.top =this.touchEndY+'px';
            ...
            touchInit(){
                ...
                this.$nextTick(()=>{
                    if(this.isSuportRefresh&&this.$route.meta.refresh){
                        this.appDom.addEventListener('touchstart',self.touchstart,false)
                        this.appDom.addEventListener('touchmove',self.touchmove,false)
                        this.appDom.addEventListener('touchend',self.touchend,false)
                        this.appDom.addEventListener('touchcancel',self.touchcancel,false)
                    }else{
                        this.appDom.removeEventListener('touchstart',self.touchstart,false) ;
                        this.appDom.removeEventListener('touchmove',self.touchmove,false)
                        this.appDom.removeEventListener('touchend',self.touchend,false)
                        this.appDom.removeEventListener('touchcancel',self.touchcancel,false)
                    }
                })
            },
            shareInit(){//微信分享sdk初始化
                ...
                if(agent!=''){
                    link = domainUrl+'/app/thirdPartyHome?agent='+localStorage.agent
                }else{
                    link = domainUrl+'/app/thirdPartyHome'
                }
...
<template>
    <div id="app">
        <div ref="content" class="content">
            <div ref="refreshTips" class="refreshIcon" v-show="$route.meta.refresh">
                <img  v-if="refreshing&&ispageLoad" src="./refresh_loading.png" class="lodingAnimat">
                <img  v-else-if="refreshend&&ispageLoad" src="./refreshend.png">
                <img v-else-if="ispageLoad&&isSuportRefresh" src="./refreshJT.png" alt="" :style="{transform:isRefresh?'rotate(180deg)':'rotate(360deg)'}">
                <span v-if="ispageLoad&&isSuportRefresh" >{{isRefreshtips}}</span>
            </div>
            <slot ></slot>
        </div>
    </div>
                ...
                this.$nextTick(()=>{
                    ...
                    for(var i=0;i<contentChild.length;i++){
                        if(contentChild[i].className.indexOf('routerView')!=-1){
                            var vappPaddingTop = window.getComputedStyle(contentChild[i])['padding-top'];
                            self.$refs.refreshTips.style.top = (Number(vappPaddingTop.slice(0,vappPaddingTop.length-2))-self.maxTouchendY)+'px';
...
<style lang="less">
    ...
    .content{
        position: relative;
        .refreshIcon{
            position: absolute;left: 0;width: 100%;height: 1rem;line-height: 1rem;text-align: center;font-size: 0.3rem;color:#707070;
            top:-1rem;
            img{
                width: 0.4rem;vertical-align: middle;margin-right: 0.2rem;
            }
        }
    }
    ...
    .refresh_T{
        position:absolute;top:-40px;left: 0;width:100%;text-align:center;z-index: 999;
    }
</style>
